...
type User struct {
	...
	// EmailAlias holds the value of the "email_alias" field.
	...
	// Usage records page display preferences keyed by admin/user page
	UsageViewPreferences map[string]interface{} `json:"usage_view_preferences,omitempty"`
...
type UserEdges struct {
	...
	// PasskeyUserHandles holds the value of the passkey_user_handles edge.
	PasskeyUserHandles []*PasskeyUserHandle `json:"passkey_user_handles,omitempty"`
...
// PasskeyUserHandlesOrErr returns the PasskeyUserHandles value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) PasskeyUserHandlesOrErr() ([]*PasskeyUserHandle, error) {
	if e.loadedTypes[2] {
		return e.PasskeyUserHandles, nil
	}
	return nil, &NotLoadedError{edge: "passkey_user_handles"}
}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldAccountTodayStatsWindows, user.FieldAPIKeyAccessTimePolicy, user.FieldUsageViewPreferences:
			values[i] = new([]byte)
...
func (_m *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.FieldEmailAlias:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field email_alias", values[i])
			} else if value.Valid {
				_m.EmailAlias = value.String
			}
		...
		case user.FieldUsageViewPreferences:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field usage_view_preferences", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.UsageViewPreferences); err != nil {
					return fmt.Errorf("unmarshal field usage_view_preferences: %w", err)
				}
			}
...
// QueryPasskeyUserHandles queries the "passkey_user_handles" edge of the User entity.
func (_m *User) QueryPasskeyUserHandles() *PasskeyUserHandleQuery {
	return NewUserClient(_m.config).QueryPasskeyUserHandles(_m)
}
...
func (_m *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("email_alias=")
	builder.WriteString(_m.EmailAlias)
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("usage_view_preferences=")
	builder.WriteString(fmt.Sprintf("%v", _m.UsageViewPreferences))
	builder.WriteString(", ")
