...
// QueryMultiGroups chains the current query on the "multi_groups" edge.
func (_q *APIKeyQuery) QueryMultiGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(apikey.Table, apikey.FieldID, selector),
			sqlgraph.To(group.Table, group.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, apikey.MultiGroupsTable, apikey.MultiGroupsPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withMultiGroups:  _q.withMultiGroups.Clone(),
...
// WithMultiGroups tells the query-builder to eager-load the nodes that are connected to
// the "multi_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithMultiGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMultiGroups = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withMultiGroups; query != nil {
		if err := _q.loadMultiGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.MultiGroups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.MultiGroups = append(n.Edges.MultiGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadMultiGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.MultiGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.MultiGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.MultiGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.MultiGroupsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "multi_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
