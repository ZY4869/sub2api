...
type APIKey struct {
	...
	// ModelDisplayMode holds the value of the "model_display_mode" field.
...
type APIKeyEdges struct {
	...
	// MultiGroups holds the value of the multi_groups edge.
	MultiGroups []*Group `json:"multi_groups,omitempty"`
...
// MultiGroupsOrErr returns the MultiGroups value or an error if the edge
// was not loaded in eager-loading.
func (e APIKeyEdges) MultiGroupsOrErr() ([]*Group, error) {
	if e.loadedTypes[2] {
		return e.MultiGroups, nil
	}
	return nil, &NotLoadedError{edge: "multi_groups"}
}
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldModelDisplayMode:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field model_display_mode", values[i])
			} else if value.Valid {
				_m.ModelDisplayMode = value.String
			}
...
// QueryMultiGroups queries the "multi_groups" edge of the APIKey entity.
func (_m *APIKey) QueryMultiGroups() *GroupQuery {
	return NewAPIKeyClient(_m.config).QueryMultiGroups(_m)
}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("model_display_mode=")
	builder.WriteString(_m.ModelDisplayMode)
	builder.WriteString(", ")
