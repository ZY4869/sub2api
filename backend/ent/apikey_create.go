...
func (_c *APIKeyCreate) AddMultiGroupIDs(ids ...int64) *APIKeyCreate {
	_c.mutation.AddMultiGroupIDs(ids...)
	return _c
}

// AddMultiGroups adds the "multi_groups" edges to the Group entity.
func (_c *APIKeyCreate) AddMultiGroups(v ...*Group) *APIKeyCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddMultiGroupIDs(ids...)
}
...
func (_c *APIKeyCreate) createSpec() (*APIKey, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.MultiGroupsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
