...
// QueryAPIKeyLinks chains the current query on the "api_key_links" edge.
func (_q *GroupQuery) QueryAPIKeyLinks() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(group.Table, group.FieldID, selector),
			sqlgraph.To(apikey.Table, apikey.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, group.APIKeyLinksTable, group.APIKeyLinksPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAPIKeyLinks:       _q.withAPIKeyLinks.Clone(),
...
// WithAPIKeyLinks tells the query-builder to eager-load the nodes that are connected to
// the "api_key_links" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithAPIKeyLinks(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAPIKeyLinks = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAPIKeyLinks; query != nil {
		if err := _q.loadAPIKeyLinks(ctx, query, nodes,
			func(n *Group) { n.Edges.APIKeyLinks = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.APIKeyLinks = append(n.Edges.APIKeyLinks, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAPIKeyLinks(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.APIKeyLinksTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(group.APIKeyLinksPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.APIKeyLinksPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.APIKeyLinksPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "api_key_links" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
