...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	captchaService := service.NewCaptchaService(settingService, turnstileService, tencentCaptchaVerifier, aliyunCaptchaVerifier)
	...
	channelMonitorService := service.ProvideChannelMonitorService(db, channelMonitorRepository, channelMonitorHistoryRepository, channelMonitorRollupRepository, settingService, secretEncryptor, configConfig, channelMonitorTemplateRepository)
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, billingCacheService, modelCatalogService, settingService, concurrencyService, configConfig)
	apiKeyAuthCacheInvalidationOutboxRepository := repository.NewAPIKeyAuthCacheInvalidationOutboxRepository(db)
	apiKeyAuthCacheInvalidator := service.ProvideAPIKeyAuthCacheInvalidator(apiKeyService, billingCacheService, apiKeyAuthCacheInvalidationOutboxRepository, db)
	...
	authService := service.ProvideAuthService(client, userRepository, authIdentityRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, captchaService, emailQueueService, promoService, affiliateService, subscriptionService)
	...
	passkeyRepository := repository.NewPasskeyRepository(client)
	passkeySessionStore := repository.NewPasskeySessionStore(redisClient)
	passkeyService := service.NewPasskeyService(configConfig, authService, passkeyRepository, passkeySessionStore)
	passkeyHandler := handler.NewPasskeyHandler(authService, passkeyService, settingService)
	...
	modelPlazaService := service.NewModelPlazaService(channelService)
	...
	adminSecurityHelper := handler.ProvideAdminSecurityHelper(totpService, auditLogService)
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, userPlatformQuotaService, totpService, auditLogService, adminSecurityHelper)
	auditLogHandler := handler.ProvideAdminAuditLogHandler(auditLogService, adminSecurityHelper)
	...
	grokTokenProvider := service.ProvideGrokTokenProvider(accountRepository, geminiTokenCache, grokOAuthService, oAuthRefreshAPI)
	...
	accountModelImportService := service.ProvideAccountModelImportService(modelCatalogService, modelRegistryService, geminiMessagesCompatService, vertexUpstreamCatalogService, openAITokenProvider, grokTokenProvider, kiroRuntimeService, httpUpstream, proxyRepository, tlsFingerprintProfileService, configConfig)
	...
	grokGatewayService := service.ProvideGrokGatewayService(gatewayService, httpUpstream, rateLimitService, grokReverseClient, configConfig, grokTokenProvider, accountRepository)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, accountModelImportService, claudeTokenProvider, openAITokenProvider, geminiTokenProvider, antigravityGatewayService, gatewayService, grokGatewayService, openAIGatewayService, geminiMessagesCompatService, userRepository, apiKeyRepository, usageLogRepository, httpUpstream, configConfig, tlsFingerprintProfileService, rateLimitService)
	channelMonitorAccountDependencyBinding := service.BindChannelMonitorAccountDependencies(channelMonitorService, accountRepository, accountTestService)
	adminChannelMonitorHandler := handler.ProvideAdminChannelMonitorHandler(channelMonitorService, channelMonitorTemplateService, channelMonitorAccountDependencyBinding)
	...
	grokQuotaService := service.NewGrokQuotaService(accountRepository, proxyRepository, grokTokenProvider, httpUpstream, usageLogRepository)
	grokQuotaFetcher := service.NewGrokQuotaFetcher()
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, usageCache, identityCache, openAIQuotaService, grokQuotaService, grokQuotaFetcher, tlsFingerprintProfileService)
	...
	accountHandler := handler.ProvideAdminAccountHandler(adminService, oAuthService, openAIOAuthService, kiroOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, accountModelImportService, accountModelDiagnosticsService, modelRegistryService, settingService, opsService, grokQuotaService, adminSecurityHelper)
	...
	dataManagementHandler := handler.ProvideAdminDataManagementHandler(dataManagementService, adminSecurityHelper)
	...
	backupService := service.ProvideBackupService(settingRepository, configConfig, secretEncryptor, backupObjectStoreFactory, dbDumper, periodicJobLeaderGate)
	backupHandler := handler.ProvideAdminBackupHandler(backupService, userService, adminSecurityHelper)
	...
	grokOAuthHandler := handler.ProvideGrokOAuthHandler(grokOAuthService, adminService, grokQuotaService, tokenRefreshService, modelRegistryService)
	antigravityOAuthHandler := admin.NewAntigravityOAuthHandler(antigravityOAuthService)
	proxyHandler := handler.ProvideAdminProxyHandler(adminService, adminSecurityHelper)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, complianceHandler, adminUserHandler, auditLogHandler, contentModerationAuditHandler, groupHandler, channelHandler, adminChannelMonitorHandler, channelMonitorTemplateHandler, accountHandler, affiliateHandler, emailTemplateHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, kiroOAuthHandler, geminiOAuthHandler, grokOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, adminAPIKeyHandler, modelCatalogHandler, modelRegistryHandler, scheduledTestHandler, tlsFingerprintProfileHandler, adminPaymentHandler)
