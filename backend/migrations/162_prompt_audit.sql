-- Prompt audit jobs and events for OpenAI-compatible guard scanning.
-- Guard credentials remain in settings; transient prompt payloads stay outside PostgreSQL.
...
CREATE TABLE IF NOT EXISTS prompt_audit_jobs (
    ...
    CONSTRAINT chk_prompt_audit_jobs_nonnegative
        CHECK (attempts >= 0 AND max_attempts >= 0 AND prompt_length >= 0 AND message_count >= 0 AND config_version >= 1)
);
