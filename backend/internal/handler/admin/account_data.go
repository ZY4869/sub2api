...
func validateDataAccount(item DataAccount) error {
	...
	switch item.Type {
	case service.AccountTypeOAuth, service.AccountTypeSetupToken, service.AccountTypeAPIKey, service.AccountTypeUpstream, service.AccountTypeSSO:
	default:
...
func enrichCredentialsFromIDToken(item *DataAccount) {
	...
	enriched, changed := service.EnrichOpenAIOAuthCredentials(item.Platform, item.Type, item.Credentials)
	if changed {
		item.Credentials = enriched
	}
}
