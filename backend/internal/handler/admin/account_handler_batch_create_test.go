...
func TestBuildBatchCreateAccountInputArchiveModeOverridesGroupBinding(t *testing.T) {
	...
	require.Equal(t, service.StatusActive, input.Status)
	require.Equal(t, service.AccountLifecycleArchived, input.LifecycleState)
...
func TestExecuteBatchCreateAccountsArchiveAutoCreatesGroupAndNamesAccounts(t *testing.T) {
	...
	require.Equal(t, service.StatusActive, adminSvc.createdAccounts[0].Status)
	require.Equal(t, service.AccountLifecycleArchived, adminSvc.createdAccounts[0].LifecycleState)
