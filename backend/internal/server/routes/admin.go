...
func registerAdminComplianceRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	if h == nil || h.Admin == nil || h.Admin.Compliance == nil {
		return
	}
...
func registerModelRegistryRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	registry := admin.Group("/models/registry")
	{
		...
		registry.POST("/move-provider", h.Admin.ModelRegistry.MoveProvider)
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.GET("/:id/platform-quotas", h.Admin.User.GetPlatformQuotas)
		users.PUT("/:id/platform-quotas", h.Admin.User.UpdatePlatformQuotas)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.POST("/profit-preview", h.Admin.Group.PreviewProfitControl)
		...
		groups.GET("/:id/composite-routes", h.Admin.Group.GetCompositeRoutes)
		groups.PUT("/:id/composite-routes", h.Admin.Group.ReplaceCompositeRoutes)
...
func registerChannelMonitorRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	monitors := admin.Group("/channel-monitors")
	{
		...
		monitors.GET("/:id/histories", h.Admin.ChannelMonitor.ListHistories)
	}

	templates := admin.Group("/channel-monitor-templates")
	{
		templates.GET("", h.Admin.ChannelMonitorTemplate.List)
		templates.GET("/:id", h.Admin.ChannelMonitorTemplate.GetByID)
		templates.POST("", h.Admin.ChannelMonitorTemplate.Create)
		templates.PUT("/:id", h.Admin.ChannelMonitorTemplate.Update)
		templates.DELETE("/:id", h.Admin.ChannelMonitorTemplate.Delete)
		templates.POST("/:id/apply", h.Admin.ChannelMonitorTemplate.ApplyToMonitor)
		templates.GET("/:id/associated-monitors", h.Admin.ChannelMonitorTemplate.ListAssociatedMonitors)
	}
}
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, panelRateLimiter *middleware.PanelRateLimiter) {
	...
	}
	{
		...
		accounts.POST("/probe-models", h.Admin.Account.ProbeModels)
		...
		accounts.POST("/:id/blacklist", h.Admin.Account.Blacklist)
		...
		accounts.POST("/blacklist/restore", h.Admin.Account.BatchRestoreBlacklisted)
		accounts.POST("/blacklist/batch-delete", h.Admin.Account.BatchDeleteBlacklisted)
		accounts.POST("/:id/blacklist/restore", h.Admin.Account.RestoreBlacklisted)
		accounts.GET("/data", h.Admin.Account.ExportData)
		accounts.POST("/data/import-jobs", h.Admin.Account.CreateImportJob)
		accounts.GET("/data/import-jobs/:job_id", h.Admin.Account.GetImportJob)
		accounts.POST("/data/import-jobs/:job_id/cancel", h.Admin.Account.CancelImportJob)
		accounts.POST("/data/import-jobs/:job_id/group-bindings", h.Admin.Account.BindImportJobGroups)
...
func registerGrokRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
...
func registerKiroOAuthRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	kiro := admin.Group("/kiro")
	{
		...
		kiro.POST("/accounts/:id/reauthorize-from-oauth", h.Admin.KiroOAuth.ReauthorizeAccountFromOAuth)
		kiro.POST("/accounts/:id/refresh", h.Admin.KiroOAuth.RefreshAccount)
	}
}
...
func registerSettingsRoutes(adminGroup *gin.RouterGroup, h *handler.Handlers, security *admin.AdminSecurityHelper) {
	adminSettings := adminGroup.Group("/settings")
	{
		...
		adminSettings.POST("/telegram/test", h.Admin.Setting.TestTelegramConnection)
...
func registerUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers, panelRateLimiter *middleware.PanelRateLimiter) {
	...
	}
	{
		...
		usage.GET("/repair-tasks", h.Admin.Usage.ListRepairTasks)
		usage.POST("/repair-tasks", h.Admin.Usage.CreateRepairTask)
		usage.POST("/repair-tasks/:id/cancel", h.Admin.Usage.CancelRepairTask)
	}
}
