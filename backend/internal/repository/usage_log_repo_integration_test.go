...
func TestUsageLogRepositoryCreate_CanceledContextReturnsError(t *testing.T) {
	client := testEntClient(t)
	repo := newUsageLogRepositoryWithSQL(client, integrationDB)
...
type usageLogFailingExecutor struct {
	err error
}

func (e usageLogFailingExecutor) ExecContext(context.Context, string, ...any) (sql.Result, error) {
	return nil, e.err
}

func (e usageLogFailingExecutor) QueryContext(context.Context, string, ...any) (*sql.Rows, error) {
	return nil, e.err
}

func TestUsageLogRepositoryCreate_ReturnsExecutorError(t *testing.T) {
	client := testEntClient(t)
	expectedErr := errors.New("insert failed")
	repo := newUsageLogRepositoryWithSQL(client, usageLogFailingExecutor{err: expectedErr})

	inserted, err := repo.Create(context.Background(), &service.UsageLog{
		UserID:       1,
		APIKeyID:     1,
		AccountID:    1,
		RequestID:    uuid.NewString(),
...
func (s *UsageLogRepoSuite) TestCreate_ConflictingRequestIDReturnsExistingID() {
	user := mustCreateUser(s.T(), s.client, &service.User{Email: "create-conflict@test.com"})
	apiKey := mustCreateApiKey(s.T(), s.client, &service.APIKey{UserID: user.ID, Key: "sk-create-conflict", Name: "k"})
	account := mustCreateAccount(s.T(), s.client, &service.Account{Name: "acc-create-conflict"})
	requestID := uuid.NewString()
	...
	inserted, err := s.repo.Create(s.ctx, first)
	s.Require().NoError(err)
	s.Require().True(inserted)

	inserted, err = s.repo.Create(s.ctx, second)
	s.Require().NoError(err)
...
func (s *UsageLogRepoSuite) TestGetAccountTodayStats() {
	...
	// account cost = SUM(total_cost_usd_equivalent * account_rate_multiplier)
	s.Require().InEpsilon(1.5, stats.Cost, 0.0001)
	// standard cost = SUM(total_cost_usd_equivalent)
