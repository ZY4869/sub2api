...
func (r *usageLogRepository) getAccountTodayStatsBreakdownBatchLegacy(ctx context.Context, accountIDs []int64, todayStart, weekStart time.Time) (map[int64]*usagestats.AccountTodayStatsBreakdown, error) {
	...
	query := `
		SELECT
			...
			COUNT(*) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)) as monthly_requests,
			COALESCE(SUM(input_tokens + output_tokens + cache_creation_tokens + cache_read_tokens) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)), 0) as monthly_tokens,
			COALESCE(SUM(total_cost_usd_equivalent * COALESCE(account_rate_multiplier, 1)) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)), 0) as monthly_cost,
			COALESCE(SUM(total_cost_usd_equivalent) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)), 0) as monthly_standard_cost,
			COALESCE(SUM(actual_cost_usd_equivalent) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)), 0) as monthly_user_cost,
			CASE WHEN COUNT(*) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)) = 0 THEN 100 ELSE ((COUNT(*) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz) AND status = 'succeeded'))::float / (COUNT(*) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz)))::float) * 100 END as monthly_success_rate,
			COALESCE(AVG(duration_ms) FILTER (WHERE created_at >= date_trunc('month', $2::timestamptz) AND status = 'succeeded' AND duration_ms IS NOT NULL), 0) as monthly_average_duration_ms,
