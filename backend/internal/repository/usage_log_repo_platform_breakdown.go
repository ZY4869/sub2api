...
func (r *usageLogRepository) queryPlatformBreakdown(ctx context.Context, filters UsageLogFilters) (results []usagestats.PlatformUsageStat, err error) {
	...
	cacheCreationExpr := usageCacheCreationSQL("ul.")
	...
	query := fmt.Sprintf(`
		SELECT
			...
			COALESCE(SUM(%s + COALESCE(ul.cache_read_tokens, 0)), 0) AS cache_tokens,
	...
	`, usagePlatformExpression, cacheCreationExpr, totalTokensExpr, usageLogPlatformJoinFromClause(), buildWhere(conditions), usagePlatformExpression)
