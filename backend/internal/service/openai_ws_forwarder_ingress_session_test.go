...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_BeforeTurnPayloadBlocksSecondTurn(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_guard_turn_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	captureDialer := &openAIWSCaptureDialer{conn: captureConn}
	pool := newOpenAIWSConnPool(cfg)
	...
	account := &Account{
		ID:          115,
		Name:        "openai-ingress-payload-hook",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	hooks := &OpenAIWSIngressHooks{
		BeforeTurnPayload: func(turn int, payload []byte, model string) error {
			scanned <- string(payload)
			require.Equal(t, "gpt-5.1", model)
			if turn == 2 {
				return NewOpenAIWSClientCloseError(coderws.StatusCode(4403), "prompt_guard_blocked", nil)
			}
			return nil
		},
	}
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false}`)
	firstTurnEvent := readMessage()
	require.Equal(t, "resp_guard_turn_1", gjson.GetBytes(firstTurnEvent, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"input":"blocked follow-up","previous_response_id":"resp_guard_turn_1"}`)
	...
	case serverErr := <-serverErrCh:
		...
		require.Equal(t, coderws.StatusCode(4403), closeErr.StatusCode())
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket 拦截结束超时")
	}
	require.Contains(t, <-scanned, "response.create")
	require.Contains(t, <-scanned, "blocked follow-up")
	require.Len(t, captureConn.writes, 1, "第二轮被审计拦截后不能写入上游")
}
