...
func isOpenAIPersonalAccessTokenAuthMode(value string) bool {
	...
	case "personalaccesstoken", "personal_access_token", "pat":
		return true
	default:
		return false
	}
...
// ValidateCodexPersonalAccessToken validates a Codex at-* token using the
// first-party endpoint used by Codex clients. The response contains routing
// metadata only; the token itself is never logged or persisted outside the
// account credentials already holding it.
func (s *OpenAIOAuthService) ValidateCodexPersonalAccessToken(ctx context.Context, accessToken, proxyURL string) (*OpenAITokenInfo, error) {
	...
	if !strings.HasPrefix(strings.ToLower(accessToken), "at-") {
	...
	client, err := httpclient.GetClient(httpclient.Options{ProxyURL: proxyURL, Timeout: 20 * time.Second, ResponseHeaderTimeout: 15 * time.Second})
	if err != nil {
...
func validateOpenAICodexPATWhoami(whoami openAICodexPATWhoamiResponse) error {
	for key, value := range map[string]string{
		"email": whoami.Email, "chatgpt_user_id": whoami.ChatGPTUserID,
		"chatgpt_account_id": whoami.ChatGPTAccountID, "chatgpt_plan_type": whoami.ChatGPTPlanType,
	} {
