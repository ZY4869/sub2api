...
func (s *SchedulerSnapshotService) rebuildByGroupIDs(ctx context.Context, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	platforms := []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformDeepSeek, PlatformKimi, PlatformOpenRouter, PlatformAntigravity, PlatformProtocolGateway}
	var firstErr error
