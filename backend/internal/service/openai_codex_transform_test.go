...
func TestApplyCodexOAuthTransform_ToolSearchOutputNormalizesCallID(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.2",
		"input": []any{
			map[string]any{"type": "tool_search_call", "call_id": "call_search_1", "query": "x"},
	...
	first, ok := input[0].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "fcsearch_1", first["call_id"])

	second, ok := input[1].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "fcsearch_1", second["call_id"])
}
...
func TestApplyCodexOAuthTransform_NormalizeCodexTools_DefaultsStrictFalse(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.1",
		"tools": []any{
			map[string]any{
...
func TestApplyCodexOAuthTransform_NormalizeCodexTools_PreservesExplicitStrict(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.1",
		"tools": []any{
			map[string]any{
