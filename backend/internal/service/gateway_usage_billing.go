...
func applyClaudeCapabilityToUsageLogFromResult(log *UsageLog, result *ForwardResult) {
	if log == nil || result == nil {
		return
	}
	log.RequestedModelRaw = optionalTrimmedStringPtr(result.RequestedModelRaw)
	log.RequestedModelNormalized = optionalTrimmedStringPtr(result.RequestedModelNormalized)
	if result.MillionContextRequested {
		value := true
		log.MillionContextRequested = &value
	}
	if result.MillionContextRequested || result.MillionContextEffective {
		value := result.MillionContextEffective
		log.MillionContextEffective = &value
	}
	log.MillionContextSource = optionalTrimmedStringPtr(result.MillionContextSource)
	log.MillionContextBetaToken = optionalTrimmedStringPtr(result.MillionContextBetaToken)
}
...
func billingCurrencyConversionFromSettings(ctx context.Context, settingService *SettingService) BillingCurrencyConversionSettings {
	...
	return settings.BillingCurrencyConversionSettings()
}
...
func applyPublicCatalogDiscountToUsageLog(usageLog *UsageLog, status *PublicModelCatalogDiscountStatus) {
	if usageLog == nil || status == nil {
		return
	}
	usageLog.DiscountApplied = status.Active
	if status.ReductionPercent > 0 {
		value := status.ReductionPercent
		usageLog.DiscountPercent = &value
	}
	usageLog.DiscountWindowID = optionalTrimmedStringPtr(status.WindowID)
	usageLog.DiscountWindowType = optionalTrimmedStringPtr(status.WindowType)
	if strings.TrimSpace(status.CompletedAt) != "" {
		if completedAt, err := time.Parse(time.RFC3339, strings.TrimSpace(status.CompletedAt)); err == nil {
			usageLog.DiscountCompletedAt = &completedAt
		}
	}
}
...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	...
	legacyReasoningEffort, reasoningEffortRaw, reasoningEffortEffective := NormalizeGatewayEffortForUsage(result.ReasoningEffort, result.ReasoningEffortRaw, result.ReasoningEffortEffective)
	usageLog := &UsageLog{UserID: user.ID, APIKeyID: apiKey.ID, AccountID: account.ID, RequestID: requestID, Model: result.Model, RequestedModel: result.Model, UpstreamModel: optionalNonEqualStringPtr(result.UpstreamModel, result.Model), ReasoningEffort: legacyReasoningEffort, ReasoningEffortRaw: reasoningEffortRaw, ReasoningEffortEffective: reasoningEffortEffective, ThinkingEnabled: thinkingEnabled, InboundEndpoint: optionalTrimmedStringPtr(input.InboundEndpoint), UpstreamEndpoint: optionalTrimmedStringPtr(input.UpstreamEndpoint), UpstreamURL: optionalTrimmedStringPtr(ResolveUsageLogUpstreamURL(account, input.UpstreamURL)), UpstreamService: optionalTrimmedStringPtr(usageProvider), InputTokens: normalizedUsage.DisplayTokens.InputTokens, ImageInputTokens: normalizedUsage.DisplayTokens.ImageInputTokens, OutputTokens: result.Usage.OutputTokens, CacheCreationTokens: normalizedUsage.DisplayTokens.CacheCreationTokens, CacheReadTokens: normalizedUsage.DisplayTokens.CacheReadTokens, CacheCreation5mTokens: normalizedUsage.DisplayTokens.CacheCreation5mTokens, CacheCreation1hTokens: normalizedUsage.DisplayTokens.CacheCreation1hTokens, InputCost: cost.InputCost, ImageInputCost: cost.ImageInputCost, OutputCost: cost.OutputCost, CacheCreationCost: cost.CacheCreationCost, CacheReadCost: cost.CacheReadCost, TotalCost: cost.TotalCost, ActualCost: actualCost, BillingCurrency: billingCurrency, TotalCostUSDEquivalent: cost.TotalCostUSDEquivalent, ActualCostUSDEquivalent: actualCostUSDEquivalent, USDToCNYRate: cost.USDToCNYRate, FXRateDate: optionalTrimmedStringPtr(cost.FXRateDate), FXLockedAt: cloneBillingTime(cost.FXLockedAt), CostByCurrency: cloneBillingStringMapFloat64(cost.CostByCurrency), ActualCostByCurrency: normalizedBillingCostMap(billingCurrency, actualCost), BillingExemptReason: billingExemptReason, RateMultiplier: multiplier, AccountRateMultiplier: &accountRateMultiplier, BillingType: billingType, Status: UsageLogStatusSucceeded, Stream: result.Stream, DurationMs: &durationMs, FirstTokenMs: result.FirstTokenMs, ImageCount: result.ImageCount, ImageSize: imageSize, CacheTTLOverridden: cacheTTLOverridden, CreatedAt: time.Now()}
	applyClaudeCapabilityToUsageLogFromResult(usageLog, result)
	if input.UserAgent != "" {
