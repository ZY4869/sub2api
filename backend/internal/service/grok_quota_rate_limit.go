...
const (
	...
	grokRateLimitRepeatCooldown      = 30 * time.Minute
	grokRateLimitSustainedCooldown   = 2 * time.Hour
	grokRateLimitMaxAdaptiveCooldown = 6 * time.Hour
)
...
func clearGrokRateLimitAfterRecovery(ctx context.Context, repo AccountRepository, account *Account) {
	...
	if recoveryRepo, ok := repo.(grokRateLimitRecoveryRepository); ok {
