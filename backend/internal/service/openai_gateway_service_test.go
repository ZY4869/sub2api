...
func TestOpenAISelectAccountForModelWithExclusions_NoModelSupport(t *testing.T) {
	...
	if !errors.Is(err, ErrOpenAIModelNotFound) {
		t.Fatalf("expected ErrOpenAIModelNotFound, got: %v", err)
	}
}
...
func TestOpenAISelectAccountWithLoadAwareness_StickyBusySelectsIdleAccount(t *testing.T) {
	sessionHash := "sticky-busy-idle"
	groupID := int64(1)
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1},
...
func TestOpenAIUpdateCodexUsageSnapshotFromHeaders_UsesFallbackRequestModel(t *testing.T) {
	repo := &snapshotUpdateAccountRepo{
		updateExtraCalls: make(chan map[string]any, 1),
		stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{{
			ID:       124,
	...
	svc.UpdateCodexUsageSnapshotFromHeaders(context.Background(), 124, headers, "gpt-5.3-codex-spark")
