...
func TestAdminService_AdminUpdateAPIKeyGroupID_UpdateFails(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "clear api key groups")
}
...
func TestAdminService_AdminUpdateAPIKeyGroupID_ModelDisplayModeOnly(t *testing.T) {
	existing := &APIKey{ID: 1, Key: "sk-test", ModelDisplayMode: APIKeyModelDisplayModeAliasOnly}
	apiKeyRepo := &apiKeyRepoStubForGroupUpdate{key: existing}
	cache := &authCacheInvalidatorStub{}
	svc := &adminServiceImpl{apiKeyRepo: apiKeyRepo, authCacheInvalidator: cache}

	mode := APIKeyModelDisplayModeSourceOnly
	got, err := svc.AdminUpdateAPIKeyGroupID(context.Background(), 1, nil, &mode)
	require.NoError(t, err)
	require.NotNil(t, apiKeyRepo.updated)
	require.Equal(t, APIKeyModelDisplayModeSourceOnly, apiKeyRepo.updated.ModelDisplayMode)
	require.Equal(t, APIKeyModelDisplayModeSourceOnly, got.APIKey.ModelDisplayMode)
	require.Equal(t, []string{"sk-test"}, cache.keys)
}
