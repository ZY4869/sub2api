...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyRateLimitedAccountFallsBackToFreshCandidate(t *testing.T) {
	...
	selection, decision, err := svc.SelectAccountWithScheduler(ctx, &groupID, "", "session_hash_rate_limited", "gpt-5.4", nil, OpenAIUpstreamTransportAny)
...
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_SkipsFreshlyRateLimitedSnapshotCandidate(t *testing.T) {
	...
	account, err := svc.SelectAccountForModelWithExclusions(ctx, &groupID, "", "gpt-5.4", nil)
	require.NoError(t, err)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyDBRuntimeRecheckSkipsStaleCachedAccount(t *testing.T) {
	...
	selection, decision, err := svc.SelectAccountWithScheduler(ctx, &groupID, "", "session_hash_db_runtime_recheck", "gpt-5.4", nil, OpenAIUpstreamTransportAny)
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyBusySelectsIdleAlternative(t *testing.T) {
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(21002), selection.Account.ID, "busy sticky account should yield to an idle account")
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_TopKBusyTriesRemainingCandidates(t *testing.T) {
	ctx := context.Background()
	groupID := int64(1101)
	accounts := []Account{
		{
	...
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Load = 0.1
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.Queue = 0.1
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.ErrorRate = 0.1
	cfg.Gateway.OpenAIWS.SchedulerScoreWeights.TTFT = 0.1

	concurrencyCache := stubConcurrencyCache{
		loadMap: map[int64]*AccountLoadInfo{
			3101: {AccountID: 3101, LoadRate: 0},
			3102: {AccountID: 3102, LoadRate: 20},
			3103: {AccountID: 3103, LoadRate: 30},
		},
		acquireResults: map[int64]bool{
			3101: false,
			3102: true,
	...
	require.Equal(t, int64(3102), cache.sessionBindings["openai:topk-busy"])
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	require.Equal(t, 3, decision.CandidateCount)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_LoadBalancePrefersLowerConcurrencyUtilization(t *testing.T) {
	ctx := context.Background()
	groupID := int64(13)
	accounts := []Account{
		{
	...
	concurrencyCache := stubConcurrencyCache{
		loadMap: map[int64]*AccountLoadInfo{
			5101: {AccountID: 5101, CurrentConcurrency: 3, LoadRate: 5, WaitingCount: 0},
			5102: {AccountID: 5102, CurrentConcurrency: 1, LoadRate: 95, WaitingCount: 0},
		},
		acquireResults: map[int64]bool{
			5101: true,
			5102: true,
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(5102), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
...
func TestOpenAIGatewayService_OpenAIAccountSchedulerMetrics(t *testing.T) {
	...
	selection, _, err := svc.SelectAccountWithScheduler(ctx, &groupID, "", "session_hash_metrics", "gpt-5.4", nil, OpenAIUpstreamTransportAny)
	require.NoError(t, err)
	require.NotNil(t, selection)
...
func TestIsOpenAIAccountCandidateBetter_PrefersUsagePressureBeforeScore(t *testing.T) {
	now := time.Now()
	fiveHourReset := now.Add(45 * time.Minute)
	sevenDayReset := now.Add(36 * time.Hour)
	fiveHourAccount := &Account{
		ID:       2001,
		Priority: 0,
		Extra: map[string]any{
			"codex_5h_used_percent": 41.0,
			"codex_5h_reset_at":     fiveHourReset.UTC().Format(time.RFC3339),
		},
	}
	sevenDayAccount := &Account{
		ID:       2002,
		Priority: 0,
		Extra: map[string]any{
			"codex_7d_used_percent": 95.0,
			"codex_7d_reset_at":     sevenDayReset.UTC().Format(time.RFC3339),
		},
	}

	require.True(t, isOpenAIAccountCandidateBetter(
		openAIAccountCandidateScore{
			account:  fiveHourAccount,
			loadInfo: &AccountLoadInfo{LoadRate: 80, WaitingCount: 2},
			pressure: buildAccountUsagePressure(fiveHourAccount, now),
			score:    0.2,
		},
		openAIAccountCandidateScore{
			account:  sevenDayAccount,
			loadInfo: &AccountLoadInfo{LoadRate: 5, WaitingCount: 0},
			pressure: buildAccountUsagePressure(sevenDayAccount, now),
			score:    0.9,
		},
	))
}

func TestIsOpenAIAccountCandidateBetter_QuotaHeadroomWeightIsNeutralByDefault(t *testing.T) {
	require.False(t, isOpenAIAccountCandidateBetter(
		openAIAccountCandidateScore{
			account:            &Account{ID: 2301, Priority: 0},
			loadInfo:           &AccountLoadInfo{},
			quotaHeadroom:      0.95,
			quotaHeadroomKnown: true,
			score:              0.1,
		},
		openAIAccountCandidateScore{
			account:            &Account{ID: 2302, Priority: 0},
			loadInfo:           &AccountLoadInfo{},
			quotaHeadroom:      0.05,
			quotaHeadroomKnown: true,
			score:              0.9,
...
func TestIsOpenAIAccountCandidateBetter_PrefersQuotaHeadroomWhenWeighted(t *testing.T) {
	require.True(t, isOpenAIAccountCandidateBetter(
		openAIAccountCandidateScore{
			account:             &Account{ID: 2401, Priority: 0},
			loadInfo:            &AccountLoadInfo{},
			quotaHeadroom:       0.8,
			quotaHeadroomKnown:  true,
			quotaHeadroomWeight: 1,
			score:               0.1,
		},
		openAIAccountCandidateScore{
			account:             &Account{ID: 2402, Priority: 0},
			loadInfo:            &AccountLoadInfo{},
			quotaHeadroom:       0.1,
			quotaHeadroomKnown:  true,
			quotaHeadroomWeight: 1,
