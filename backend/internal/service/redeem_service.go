...
var (
	...
	ErrRedeemCodeUnsupportedType = infraerrors.BadRequest("REDEEM_CODE_UNSUPPORTED_TYPE", "invitation codes can only be used during registration")
)
...
func (s *RedeemService) Redeem(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	...
	if redeemCode.Type == RedeemTypeInvitation {
		s.incrementRedeemErrorCount(ctx, userID)
		return nil, ErrRedeemCodeUnsupportedType
	}
