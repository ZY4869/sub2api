...
- 只读演示模式：可传 `catalog_mode=demo` 请求演示目录；只有服务端配置 `PUBLIC_MODEL_CATALOG_DEMO_MODE=true` 时才会返回 demo 条目。默认真实模式会过滤 `is_demo=true` / `catalog_entry_source=demo` 条目，避免演示快照混入正式目录。
- 鉴权：无需登录，游客与已登录用户都可访问
- 用途：返回前台 `/models` 页面使用的“平台提供”目录，包含供应商、请求协议族、公开条目售卖价、官方参考价、倍率摘要，以及目录配置中的 `page_size`
- 发布语义：有已发布快照时，接口固定返回最近一次“推送更新”冻结下来的正式目录，列表内容、排序、分页大小、价格和详情示例都会以该版本为准
...
- 详情入口里的 `:model` 必须是公开响应中的 `public_model_id` / `model`，不会接受 `source_model_id`、`source_ids`、真实账号 ID 或内部路由 ID 作为查询别名
- 可用性语义：公开目录只表达平台发布承诺，不代表任意站内 Key 都可调用。运行时、`/v1/models`、Key 目录和模型详情仍会按当前 API Key、分组、账号绑定、生图专用限制、余额 / 配额 / 订阅与当前可路由状态收敛。
- 快照新鲜度：发布时写入 `published_at` 和 `last_revalidated_at`；后续手动或后台自动检测只更新 `last_revalidated_at` 与 `stale_reason`，不会自动下架模型，也不会自动改写发布价格、上下文或示例。
- 缓存：响应会返回 `ETag`，客户端可通过 `If-None-Match` 复用 `304 Not Modified`

这个接口只暴露展示所需的公共目录数据，不替代具体协议页中的 `/v1/models`、`/v1beta/models` 等运行时模型枚举接口。Key 级可用目录使用下面两个接口：

- 登录用户视图：`GET /api/v1/keys/:id/model-catalog?include_unavailable=0|1`，只能访问当前用户自己的 Key。
- 站内 API Key 视图：`GET /v1/model-catalog?include_unavailable=0|1`，按当前 Bearer / Google API key 解析同一套 Key 目录。
- 默认返回值：`include_unavailable=0` 只返回当前 Key 可调用的已发布公开模型交集；Key 未绑定单独模型时，空 `model_patterns` 表示可调用全部已发布公开模型。
- 不可用解释：`include_unavailable=1` 会保留不可用项，并在条目上返回 `key_availability=unavailable` 与 `unavailable_reason`。原因至少包括 `not_selected_by_key`、`group_unavailable`、`image_only_key_restricted`、`published_source_unavailable`。
- 生图专用：当站内 Key 开启“生图专用”后，Key 目录、`/v1/models`、`/v1beta/models`、模型详情和运行时路由都会继续收敛，只暴露原生生图模型；非图片模型在 `include_unavailable=1` 时会标注 `image_only_key_restricted`。

公共目录与运行时模型枚举接口共用同一套账号模型投影规则，必须记住下面几条：
...
- 模型列表读路径只读取本地策略投影和本地 availability snapshot。即使 snapshot 缺失或过期，也只会返回现有投影并标记健康状态，不会在读请求里同步触发实时探测。
- 没有有效展示价的模型不会进入发布候选。候选展示仍可按“sale 优先、缺失字段逐项回退 official”的生效展示价生成草稿，方便管理员补价；但发布前会做计费覆盖性校验，售卖价缺少运行时必要字段时禁止发布。
...
- `published_at`：发布快照生成时间；实时回退时可能为空
- `last_revalidated_at`：最后一次发布快照检测时间；未检测时可能为空
- `stale_reason`：最近一次检测得到的失效原因摘要；空字符串表示暂无失效提示
- `page_size`：公开模型库前台默认每页数量；命中已发布版本时使用发布快照中的固定值，回退实时时使用当前默认页大小
- `catalog_source`：目录来源，固定为 `published` 或 `live_fallback`
- `items[].publication_status`：公开发布状态，目前为 `published`
- `items[].health_status`：公开健康摘要，固定为 `healthy` / `warning` / `error` / `pending`
- `items[].verification_source`：公开验证来源，固定为 `published_snapshot` 或 `live_fallback`
- `items[].lifecycle_status`：兼容生命周期摘要，固定为 `stable` / `beta` / `deprecated`
- `items[].lifecycle`：结构化生命周期元数据，包含 `status`、`source`、`confidence`。`source=inferred` 表示仅由名称或历史快照推断，前端必须降级展示，不能当作供应商实时生命周期。
...
- `items[].context_window_tokens`：兼容上下文窗口摘要，等于 `items[].context_window.tokens`
- `items[].context_window`：结构化上下文元数据，包含 `tokens`、`source`、`verified`、`last_checked_at`、`limit_kind`、`notes[]`。`pricing_catalog` / `official_registry` / `manual_config` 只表示声明来源；只有 `verified=true` 或 `source=verified_probe|runtime_observed` 才能展示为已验证能力。
- `items[].modalities`、`items[].capabilities`：兼容展示摘要；`capabilities` 由 `capability_matrix` 中 `supported|partial` 项聚合，旧快照缺矩阵时才回退旧字段。
- `items[].capability_matrix[]`：模型级能力矩阵，每项包含 `capability`、`protocol`、`endpoint`、`support`、`mode`、`source`、`verified`、`last_checked_at`、`limitations[]`。该字段描述模型在特定协议和 endpoint 下的能力状态，不能与平台路由矩阵混用。
- `items[].protocol_endpoints[]`：协议 endpoint 粒度支持状态，每项包含 `key`、`protocol`、`endpoint`、`method`、`support`、`source`、`verified`、`last_checked_at`、`limitations[]`。示例 key 包括 `openai.chat.completions`、`openai.responses`、`openai.embeddings`、`openai.images.generations`、`anthropic.messages`、`gemini.generateContent`、`gemini.countTokens`、`gemini.images.generations`。由平台 `protocol_capability_matrix` 或旧 `request_protocols` 推导出的 endpoint 只能作为候选，必须保留 `verified=false`；只有 `runtime_observed`、`verified_probe` 或账号探测确认过的项才能展示为已验证。
- `items[].request_protocols`：兼容协议摘要，只表示该协议至少有一个 `supported|partial` endpoint，不代表完整支持该协议所有接口。
- 合并优先级固定为 `runtime_observed` > `verified_probe` > `account_probe` > `official_registry` / `manual_config` > `published_snapshot` > `pricing_catalog` > `inferred`。更高优先级的 `unsupported` 会覆盖声明来源的 `supported`，且不会进入 `capabilities` / `request_protocols` legacy summary。
- `items[].is_demo` / `items[].catalog_entry_source`：目录来源标记；真实模式默认过滤 demo 条目，真实发布继续拒绝 `is_demo=true` 或 `catalog_entry_source=demo` 并返回 `PUBLIC_MODEL_DEMO_ENTRY_FORBIDDEN`；只有配置 `PUBLIC_MODEL_CATALOG_DEMO_MODE=true` 且查询 `catalog_mode=demo` 时，公开目录、详情接口和管理员草稿候选才会只读展示 demo 条目。
- 公开响应不会返回 `status`、`availability_state`、`stale_state`、`source_account_id`、`source_account_name`、`source_ids`、`source_model_id`、`source_protocol` 或 `source_alias`。这些字段只存在于后台候选、发布快照内部、路由和诊断链路中。
- `items[].price_display.primary`：兼容字段，等同本公开条目的售卖价核心展示行；文本模型可能包含 `input_price`、`output_price`、`cache_creation`、`cache_read`、`cache_5m`、`cache_1h`，图片 / 视频 / 请求类模型按实际单位返回 `output_price` 或补充计费项
- `items[].price_display.secondary`：附加售卖价行，只保留 batch、grounding、retrieval 等补充项
- `items[].official_price_display`：官方参考价展示行，用于前台或后台对比展示；缺失时前端应展示“未配置官方参考价”，不能把售卖价误称为官方价 / 基础价
- `items[].sale_price_display`：本公开条目的售卖价展示行；实际计费命中发布条目时使用这组价格
- 价格行字段：每个 `PublicModelCatalogPriceEntry` 都会返回 `id`、`unit`、`value`，并尽量带上 `unit_kind` 与 `display_unit`。`unit_kind` 固定为 `token` / `image` / `request` / `video` 等展示类别；`display_unit` 固定为 `per_million_tokens` / `per_image` / `per_request` / `per_video` 等前端展示单位。`configured=false` 且 `supported_unpriced=true` 表示模型支持该能力但价格未配置，例如“缓存支持，价格未配置”。
...
- 返回字段：`items[].public_model_id`、`items[].model`、`items[].aliases`、`health_status`、`health_source`、`status_reason`、`success_rate_today`、`success_rate_7d`、`latency_ms`、`last_checked_at`、`daily[]`、`trend[]`
- `items[].model` 继续等于公开可调用 ID，用于兼容旧客户端；新客户端应优先使用 `public_model_id` 匹配目录项，`aliases` 仅包含安全的公开 ID 兼容别名，不包含真实来源模型 ID
- `health_source` 固定为 `traffic` / `probe` / `none`：`traffic` 表示真实用户请求聚合，`probe` 表示 channel monitor 探针聚合，`none` 表示没有可展示的实时健康数据；成功率与延迟必须按该来源解释，不能把探针数据当成真实请求 SLA
- `status_reason` 固定为 `traffic_recent` / `probe_recent` / `monitor_disabled` / `no_history` / `stale_history` / `checking`，用于区分未配置监控、暂无历史、数据陈旧和正在检测
- `stale_history` 可以来自过期的真实请求历史或过期的探针历史；此时只返回 `last_checked_at` 表示最后观测时间，不返回成功率、延迟、`daily` 或 `trend` 作为有效 SLA 指标
- 公共状态只返回模型级健康状态和脱敏 SLA / 趋势字段，不返回 `rate_limit`、账号池容量来源、API key 限额、group 配额、用户平台配额、账号限流 / 冷却、Anthropic RPM / Gemini quota 等内部容量明细；完整来源只在管理员诊断接口展示
- 脱敏规则：不会返回账号 ID、监控器 ID、`source_model_id`、`source_ids`、内部路由 ID、上游原始错误、API Key、Header 或账号名称；真实来源模型只在后端内部用于健康归并和运行时路由
...
- 详情页调用示例基于 `protocol_endpoints` 和 `capability_matrix` 选择 endpoint。没有 `supported|partial` endpoint 时不会生成误导示例；未真实调用的发布期校验会标记 `example_validation=dry_run_contract`。
- 当运行时明确命中 endpoint / capability / context 不支持，系统会异步、节流地把该失败观察回写到发布快照能力矩阵；普通 `429`、网络错误、账号过期或临时上游故障不会覆盖为 `unsupported`。
- 能力契约校验与运行时回写会计入 Protocol Gateway 运行时指标：`model_capability_verification_total` 和 `model_capability_verification_by_result`，结果分桶为 `success`、`failure`、`skipped`。
- `source_model_id`、`base_model` 和后台候选里的真实账号信息只用于内部路由和诊断，不会作为额外 public ID 暴露给客户端调用。
- 计费命中发布条目时使用 `sale_price_display` / `runtime_price_spec`，并在内部 billing metadata 记录 `entry_id`；发布条目的售价已经是最终售卖价，不再乘站内分组倍率或用户专属倍率。发布前会阻断缺字段，因此正常发布快照不会进入缺价路径；如果发布条目售价为空或无法覆盖实际用量，运行时会 fail-closed 并返回 / 记录 `public_catalog_entry_incomplete`，不会静默回退到 legacy pricing 扣费。旧快照中的历史 `cache_price` 只作为运行时缓存价兼容读取，命中时记录 `public_catalog_legacy_cache_price_used`，新发布不能用它代替 `cache_creation`、`cache_read`、`cache_5m`、`cache_1h`
- 公开目录和分组目录展示的已发布价格都是发布时冻结的价格，不会按 `/groups/model-catalog` 分组倍率改写；分组倍率只在没有发布快照的实时回退目录展示价或未命中发布条目的旧计费路径中继续参与计算。
...
```json
{
  ...
  "published_at": "2026-04-21T10:05:00Z",
  "last_revalidated_at": "2026-04-21T10:05:00Z",
  "stale_reason": "",
  "page_size": 10,
  ...
  "items": [
    {
      ...
      "health_status": "healthy",
      "verification_source": "published_snapshot",
      "lifecycle_status": "stable",
      "lifecycle": { "status": "stable", "source": "published_snapshot", "confidence": "declared" },
      ...
      "price_display": {
        "primary": [
          ...
          { "id": "cache_creation", "unit": "cache_create_token", "unit_kind": "token", "display_unit": "per_million_tokens", "value": 0.0000003, "configured": true },
          { "id": "cache_read", "unit": "cache_read_token", "unit_kind": "token", "display_unit": "per_million_tokens", "value": 0.0000001, "configured": true },
          { "id": "cache_5m", "unit": "cache_create_token", "unit_kind": "token", "display_unit": "per_million_tokens", "value": 0.0000003, "configured": true },
          { "id": "cache_1h", "unit": "cache_storage_token_hour", "unit_kind": "token", "display_unit": "per_million_tokens", "value": 0.0000006, "configured": true }
        ],
        "secondary": [
          { "id": "batch_cache_price", "unit": "cache_create_token", "unit_kind": "token", "display_unit": "per_million_tokens", "value": 0.00000015, "configured": true },
...
`GET /api/v1/keys/:id/model-catalog?include_unavailable=1` 的典型条目会额外包含 Key 视图字段：
  ...
  "items": [
    {
      ...
      "unavailable_reason": "not_selected_by_key",
      "currency": "USD",
...
已登录用户另外还有一个用于 Key 创建 / 编辑器的辅助接口：

- 路径：`GET /api/v1/groups/model-options`
- 鉴权：必须登录
- 用途：返回当前用户可绑定分组下、且当前具备有效价格的公开模型列表；普通用户保存 Key 时会把结构化勾选结果写回 `groups[].model_patterns`
- 默认策略：空 `model_patterns` 表示这个 Key 在该分组下可以调用全部已发布公开模型；如果用户选择了具体模型，仍然会校验为可见 public model ID。
- 选择约束：普通用户提交的非空 `model_patterns` 只能是该用户在该分组可见的 public model ID；通配符、任意文本和不可见模型会被拒绝。
...
- 回退语义：如果正式公开目录尚未发布，这个接口会和前台 `/models` 一样自动回退到实时目录，但仍会继续过滤掉没有有效展示价的模型；再次发布时仍必须补齐售卖价覆盖字段

管理员后台 `/api/v1/admin/billing/public-model-catalog/draft` 会额外返回 `available_entries[].source_account_id` 与 `available_entries[].source_account_name`，仅用于后台识别账号来源；这些字段不会出现在公开 `/meta/model-catalog`、详情、分组目录或模型枚举响应里。草稿候选同样支持只读 `catalog_mode=demo`，但仅在 `PUBLIC_MODEL_CATALOG_DEMO_MODE=true` 时返回 demo 条目，且发布接口无论是否开启 demo mode 都会拒绝 demo 条目。保存草稿时推荐提交 `selected_entries[]`，每个条目包含 `entry_id`、`public_model_id`、`source_alias`、`source_account_id`、`source_protocol`、`source_model_id` 与 `sale_price_display`；旧前端仍可提交 `selected_models[]`，后端会按公开模型 ID 自动兼容迁移。发布时会校验 `public_model_id` 唯一、售价非负、条目仍存在于当前候选快照，并对 `sale_price_display` 做计费覆盖性校验：文本模型必须覆盖 input / output，支持缓存时还必须显式覆盖 `cache_creation`、`cache_read`、`cache_5m`、`cache_1h`，不能用旧 `cache_price` 代替；图片 / 视频 / 请求类模型按运行时 `RuntimePriceSpec.OutputChargeSlot` 和 `unit_kind` / `display_unit` 覆盖 `output_price`、`request`、`image` 或 `video` 价格。如果候选快照刷新导致 `entry_id` 变化，但同一 `source_account_id + source_protocol + source_model_id/base_model` 仍唯一存在，后端会把草稿条目重新对齐到当前候选条目；失败会返回 `PUBLIC_MODEL_ENTRY_UNAVAILABLE`、`PUBLIC_MODEL_ID_REQUIRED`、`PUBLIC_MODEL_ID_DUPLICATE`、`PUBLIC_MODEL_PRICE_INVALID`、`PUBLIC_MODEL_DEMO_ENTRY_FORBIDDEN` 或 `PUBLIC_MODEL_BILLING_INCOMPLETE`，避免静默发布部分条目。`PUBLIC_MODEL_BILLING_INCOMPLETE` 会在 `metadata.public_model_ids`、`metadata.missing_fields` 和 `metadata.missing_fields.<public_model_id>` 中列出缺失字段，字段名使用 `cache_creation` / `cache_read` / `cache_5m` / `cache_1h` 等运行时分项，供管理员修复。发布接口会写入 `published_at=now`、`last_revalidated_at=now`、`stale_reason=""`。

管理员后台“对外模型展示”页还提供发布快照检测治理：

- `GET /api/v1/admin/billing/public-model-catalog/revalidation`：读取“自动检测已发布模型是否失效”开关，默认关闭。
- `PUT /api/v1/admin/billing/public-model-catalog/revalidation`：更新 `auto_enabled`。
- 开启 `auto_enabled` 后，后台任务会按约 24 小时周期检查已发布快照是否仍可路由；关闭时后台任务只空转，不会改写快照。
- `POST /api/v1/admin/billing/public-model-catalog/revalidate`：手动立即检测已发布模型是否仍可路由。检测失败不会自动下架、不自动重推、不改写价格或示例，只更新 `last_revalidated_at` 与 `stale_reason`，供管理员处理。
- `GET /api/v1/admin/billing/public-model-catalog/diagnostics`：管理员专用的模型健康度容量诊断。返回 `public_model_id` 维度的 `availability`、`effective_rate_limit`、`restrictions[]`、`sources[]` 和汇总计数，用于排查 API key 限额、group / 用户平台配额、账号池调度限制、账号限流 / 冷却、Anthropic RPM / Gemini quota 等容量来源。`restrictions[].scope` 固定为 `account` / `model` / `api_key` / `group` / `user_platform` / `provider_quota`；响应不得包含 API key、token、Header 或上游原始敏感信息；这些来源明细不会出现在公共 `/meta/model-catalog/status`。

管理员后台“官方参考价/价格源管理”页只维护官方参考价、缺价审计与导入导出。该页导入旧 JSON 时会忽略 `patch.sale`，并提示前往“对外模型展示”页维护售卖价；新导出的补丁模板也只包含 `patch.official`。旧模型级 sale 接口仍保留为兼容层和未命中公开条目的 fallback，但会记录弃用日志与指标，前端不再把它作为售卖价主入口；已命中发布公开条目的运行时计费不会因发布价字段缺失而静默使用旧模型级价格。

如果前台已经处在明确的分组上下文里，还可以使用分组目录接口：

- 路径：`GET /api/v1/groups/model-catalog?group_id=<GROUP_ID>`
- 鉴权：必须登录
- 用途：返回当前公开模型目录在指定分组上下文下的展示结构；如果存在已发布快照，价格保持发布时冻结值，不按分组倍率改写；没有已发布快照时才回退实时目录并按当前分组倍率计算展示价。
- 约束：这里只用于用户自己的分组上下文页面；真实运行时仍按 API Key、分组、账号策略、余额和可服务性校验。
下面的例子分别展示三种常用认证写法。
...
- 公共目录、分组目录和后台价格页里的 `currency` 是源币种；`price_display.primary[].value` 按该源币种展示。候选展示可使用“sale 优先、缺失字段逐项回退 official”的生效表单，发布快照和运行时计费必须以完整 `sale_price_display` 覆盖为准；sale 层倍率只作用于 sale 自身已填写字段，不会套到 official 回退字段上。CNY 官方参考价保持人民币数值，不会在保存或展示时先折算成 USD。
...
| OpenAI Embeddings 兼容 | `/v1/embeddings`、`/embeddings` | 仅调度 OpenAI API-key 账号；账号显式配置 `openai_capabilities` 时需要包含 `embeddings` |
...
- `GET /api/v1/admin/ops/runtime/protocol-gateway`：返回协议网关运行时指标快照，包含路由不匹配、协议不支持、本地化 fallback、账号测试/探测解析失败、公开模型投影、公开模型限制命中、能力契约校验与运行时能力回写结果等计数。
