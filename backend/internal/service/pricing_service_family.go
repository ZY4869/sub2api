...
func (s *PricingService) matchByModelFamily(model string) *LiteLLMModelPricing {
	...
	families := []modelFamily{
		{name: "opus-5", match: []string{"claude-opus-5", "claude-opus-5.0"}},
	...
	if matched == nil {
		...
		case strings.Contains(model, "opus"):
			switch {
			case strings.Contains(model, "5.0") || strings.Contains(model, "5-0") || strings.Contains(model, "opus-5") || strings.Contains(model, "opus5"):
				fallbackName = "opus-5"
