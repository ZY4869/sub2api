...
func TestVerifyPendingOAuthToken_ValidToken(t *testing.T) {
	...
	token, err := svc.CreatePendingOAuthToken("user@example.com", "alice", "AFFCODE123")
	require.NoError(t, err)
	require.NotEmpty(t, token)

	email, username, affCode, err := svc.VerifyPendingOAuthToken(token)
	require.NoError(t, err)
	require.Equal(t, "user@example.com", email)
	require.Equal(t, "alice", username)
	require.Equal(t, "AFFCODE123", affCode)
}
