...
func TestSettingService_UpdateSettings_RegistrationEmailSuffixWhitelist_Normalized(t *testing.T) {
	...
	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		RegistrationEmailSuffixWhitelist: []string{"example.com", "@EXAMPLE.com", " @foo.bar ", "@*.Sub.test"},
	})
	require.NoError(t, err)
	require.Equal(t, `["@example.com","@foo.bar","@*.sub.test"]`, repo.updates[SettingKeyRegistrationEmailSuffixWhitelist])
}
