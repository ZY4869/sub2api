...
func TestExplainSeedResolutionReportsDeprecatedReplacement(t *testing.T) {
	...
	require.Equal(t, "claude-haiku-4.5", resolution.CanonicalID)
	require.Equal(t, "claude-haiku-4.5", resolution.EffectiveID)
	require.False(t, resolution.Deprecated)
	require.Nil(t, resolution.ReplacementEntry)
	require.Equal(t, "pricing", resolution.MatchedBy)
	require.Equal(t, "claude-haiku-4-5-20251001", resolution.MatchedValue)
}
