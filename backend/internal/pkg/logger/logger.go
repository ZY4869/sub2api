...
func buildLogger(options InitOptions) (*zap.Logger, zap.AtomicLevel, []io.Closer, error) {
	...
	if options.Output.ToStdout {
		...
		cores = append(cores, zapcore.NewCore(enc, zapcore.Lock(noSyncWriteSyncer{Writer: os.Stdout}), infoPriority))
		cores = append(cores, zapcore.NewCore(enc, zapcore.Lock(noSyncWriteSyncer{Writer: os.Stderr}), errPriority))
	}
	...
	if len(cores) == 0 {
		cores = append(cores, zapcore.NewCore(enc, zapcore.Lock(noSyncWriteSyncer{Writer: os.Stdout}), atomic))
	}
