...
func BuildBillingSummary(config *BillingConfig) *BillingSummary {
	...
	if used != nil {
		if monthlyLimit != nil && *monthlyLimit > 0 {
			value := math.Min(*used, *monthlyLimit)
			includedUsed = &value
		} else {
	...
	hasWeekly := config.CreditUsagePercent != nil || periodType == "weekly" || len(products) > 0
	hasMonthly := monthlyLimit != nil || used != nil || (!hasWeekly && periodEnd != "")
	...
	if hasWeekly {
		...
		summary.UsagePercent = cloneFloat(config.CreditUsagePercent)
	...
	if hasMonthly {
		summary.BillingPeriodStart = strings.TrimSpace(config.BillingPeriodStart)
		summary.BillingPeriodEnd = strings.TrimSpace(config.BillingPeriodEnd)
	}
