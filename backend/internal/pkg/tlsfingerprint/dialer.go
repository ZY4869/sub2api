...
func NewHTTPProxyDialer(profile *Profile, proxyURL *url.URL) *HTTPProxyDialer {
	return NewHTTPProxyDialerWithBaseDialer(profile, proxyURL, nil)
}

// NewHTTPProxyDialerWithBaseDialer creates an HTTP proxy TLS dialer with a
// caller-provided dialer for connecting to the proxy server itself.
func NewHTTPProxyDialerWithBaseDialer(profile *Profile, proxyURL *url.URL, baseDialer func(ctx context.Context, network, addr string) (net.Conn, error)) *HTTPProxyDialer {
	if baseDialer == nil {
		baseDialer = (&net.Dialer{}).DialContext
	}
...
func NewSOCKS5ProxyDialer(profile *Profile, proxyURL *url.URL) *SOCKS5ProxyDialer {
	return NewSOCKS5ProxyDialerWithForwardDialer(profile, proxyURL, proxy.Direct)
}

// NewSOCKS5ProxyDialerWithForwardDialer creates a SOCKS5 TLS dialer with a
// caller-provided dialer for connecting to the proxy server itself.
func NewSOCKS5ProxyDialerWithForwardDialer(profile *Profile, proxyURL *url.URL, forward proxy.Dialer) *SOCKS5ProxyDialer {
	if forward == nil {
...
func (d *HTTPProxyDialer) DialTLSContext(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	baseDialer := d.baseDialer
	if baseDialer == nil {
		baseDialer = (&net.Dialer{}).DialContext
	}
	conn, err := baseDialer(ctx, "tcp", proxyAddr)
	if err != nil {
