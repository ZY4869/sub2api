...
- 当前已支持的 Gemini 资源族，已经严格对齐仓库内置的 `2026-04-15` 官方 discovery 真值表：`models`、`files`、`batches`、`cachedContents`、`fileSearchStores`、`interactions`、`live authTokens`、`corpora`、`corpora.permissions`、`dynamic`、`generatedFiles`、`generatedFiles.operations`、`models.operations`、`tunedModels`、`tunedModels.permissions`、`tunedModels.operations`、Gemini OpenAI compat，以及现有 Vertex Gemini 路由。
- 当前主推的官方入口包括：`/v1/models`、`/v1alpha/authTokens`、`/v1beta/fileSearchStores/...`、`/upload/v1beta/files`、`/upload/v1beta/fileSearchStores/...`，以及上述 discovery 当前列出的各 `v1beta` 官方资源族路径。
- 当前 discovery 中已无额外被本仓库故意留空的官方 Gemini 资源族。
- 在方法级别，当前 discovery 中的非历史/非废弃官方方法也已补齐；仅剩显式列出的废弃/历史方法保持不支持。
- `models.generateAnswer` 按 discovery 真值纳入支持；若 `all-methods` 页面暂未同步列出，视为官方文档漂移，而不是本仓库自定义扩展。
- `interactions` 仍视为 beta/preview surface。网关保持 passthrough-first，遇到未知响应字段或新增 `usage.*` 子字段时会继续透传，不会因为本地 schema 落后而拦截。
- `models.list/get` 仍优先透传上游官方 metadata；当上游 metadata 不可用时，fallback 会继续返回当前 API key 可见模型，但不确定字段会留空或省略，`supportedGenerationMethods` 仅在确认能力时返回，分页继续使用 opaque `nextPageToken`，维持最小壳对象策略。
- 旧 alias 仍保留一个过渡版本用于兼容：`/v1beta/live/auth-token`、`/v1beta/live/auth-tokens`、`/v1beta/documents`、`/v1beta/operations`。这些 alias 不再作为文档主路径，也不会继续扩展新能力。
- 当前明确保持不支持的废弃/历史方法：`generateText`、`generateMessage`、`countTextTokens`、`countMessageTokens`、`embedText`、`batchEmbedText`、`predict`、`predictLongRunning`。
...
<a href="https://star-history.com/#ZY4869/sub2api&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=ZY4869/sub2api&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=ZY4869/sub2api&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=ZY4869/sub2api&type=Date" />
 </picture>
</a>
