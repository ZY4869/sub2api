...
// Shared model/preset sources
const supportedModelPlatforms: AccountPlatform[] = ['anthropic', 'antigravity', 'openai', 'gemini', 'sora']
...
const visiblePlatforms = computed(() => {
  const platforms = props.selectedPlatforms.filter(platform => supportedModelPlatforms.includes(platform))
...
const presetMappings = computed<PresetMapping[]>(() => {
  const presets = visiblePlatforms.value.flatMap(platform => getPresetMappingsByPlatform(platform))
