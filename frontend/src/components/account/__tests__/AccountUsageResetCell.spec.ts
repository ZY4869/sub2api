...
describe('AccountUsageResetCell', () => {
  ...
  it('renders separate reset rows with dynamic 5H and 30D window labels', async () => {
    ...
    expect(wrapper.get('[aria-label="02H 53M"]').exists()).toBe(true)
    expect(wrapper.text()).not.toContain('02H:53M')
    ...
    expect(wrapper.get('[aria-label="06D 13H"]').exists()).toBe(true)
    expect(wrapper.text()).not.toContain('06D:13H')
    ...
    const segments = wrapper.findAll('[data-testid="account-usage-reset-countdown-segment"]')
    expect(segments[0].text()).toBe('02H')
    expect(segments[0].classes()).toContain('bg-indigo-100')
    expect(segments[0].classes()).toContain('font-bold')
    expect(segments[1].text()).toBe('53M')
    expect(segments[1].classes()).toContain('bg-sky-100')
    expect(segments[2].text()).toBe('06D')
    expect(segments[2].classes()).toContain('bg-emerald-100')
    expect(segments[3].text()).toBe('13H')
    expect(segments[3].classes()).toContain('bg-indigo-100')
  ...
  it('keeps pro openai normal and spark reset rows aligned with their labels', async () => {
    ...
    expect(wrapper.get('[aria-label="01H 00M"]').exists()).toBe(true)
    expect(text).not.toContain('01H:00M')
    expect(text).toContain('13:00:00')
    expect(text).not.toContain('Today 13:00:00')
    expect(text).toContain('30D')
    expect(wrapper.get('[aria-label="02H 00M"]').exists()).toBe(true)
    expect(text).not.toContain('02H:00M')
    expect(text).toContain('14:00:00')
    expect(text).not.toContain('Today 14:00:00')
    expect(text).toContain('Spark 5H')
    expect(wrapper.get('[aria-label="03H 00M"]').exists()).toBe(true)
    expect(text).not.toContain('03H:00M')
    expect(text).toContain('15:00:00')
    expect(text).not.toContain('Today 15:00:00')
    expect(text).toContain('Spark 30D')
    expect(wrapper.get('[aria-label="04H 00M"]').exists()).toBe(true)
    expect(text).not.toContain('04H:00M')
  ...
  it('treats 31D reset labels as monthly green capsules', async () => {
    ...
    expect(wrapper.get('[aria-label="06D 14H"]').exists()).toBe(true)
    expect(wrapper.text()).not.toContain('06D:14H')
  ...
  it('keeps cross-year long reset countdown and full tooltip readable', async () => {
      ...
      props: {
        account: {
          ...
          extra: {
            ...
            codex_7d_reset_at: '2027-01-02T01:15:00',
          },
        } as any,
      },
    })
    ...
    expect(wrapper.get('[aria-label="01D 14H"]').exists()).toBe(true)
    expect(wrapper.text()).not.toContain('01D:14H')
    expect(wrapper.text()).toContain('2027-01-02 01:15:00')
    expect(wrapper.get('[title="2027-01-02 01:15:00"]').exists()).toBe(true)
  })
  ...
  it('updates day labels when the shared clock crosses midnight', async () => {
    ...
    await flushPromises()
    expect(wrapper.get('[aria-label="00H 02M"]').exists()).toBe(true)
    expect(wrapper.text()).not.toContain('00H:02M')
  ...
  it('anchors API remaining_seconds so reset countdown ticks down in realtime', async () => {
    ...
    await flushPromises()
    expect(wrapper.get('[aria-label="00H 03M"]').exists()).toBe(true)
    expect(wrapper.text()).not.toContain('00H:03M')
    expect(wrapper.text()).toContain('-')
    expect(wrapper.text()).not.toContain('12:03:00')
