<template>
  <div class="space-y-4">
    ...
    <div class="rounded-lg border border-emerald-200 bg-white/80 p-4 dark:border-emerald-900/40 dark:bg-slate-900/60">
      ...
      <div v-if="deviceInputDetected" class="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-200">
        {{ t('admin.accounts.grokOauth.externalDeviceCodeHint') }}
      </div>
...
const parsedKind = ref('unknown')
const errorMessage = ref('')
const loading = ref(false)
const suppressNextCallbackWatch = ref(false)

const resolvedState = computed(() => parsedState.value || authState.value?.state || '')
const deviceInputDetected = computed(() => parsedKind.value === 'device_user_code' || parsedKind.value === 'device_url')
const canSubmit = computed(() => Boolean(authState.value?.session_id && parsedCode.value && resolvedState.value && !deviceInputDetected.value))

watch(callbackUrl, (value) => {
  const parsed = parseGrokAuthorizationInput(value)
  parsedKind.value = parsed.kind
  ...
  if (deviceInputDetected.value) {
    errorMessage.value = t('admin.accounts.grokOauth.externalDeviceCodeHint')
    emit('deviceInput')
    return
  }
  errorMessage.value = ''
...
async function generateAuthUrl() {
  ...
  errorMessage.value = ''
  try {
    authState.value = await adminAPI.accounts.generateGrokAuthUrl({ proxy_id: props.proxyId })
    callbackUrl.value = ''
    parsedCode.value = ''
    parsedState.value = ''
    parsedKind.value = 'unknown'
  } catch (error: any) {
