<template>
  <div
    ref="rootRef"
    class="flex min-w-[150px] flex-col justify-center gap-1.5"
    data-testid="account-usage-visual-cell"
  >
    <div v-if="tierBadges.length > 0" class="flex flex-wrap items-center gap-1">
      <span
    ...
    <div v-else-if="displayRows.length > 0" class="space-y-1.5">
      <div
        ...
        :key="row.key"
        class="group flex items-center gap-2 text-[11px]"
        :title="rowTitle(row)"
      >
        <span :class="['w-7 shrink-0 rounded border px-0.5 py-[1px] text-center text-[9px] font-bold', rowTagClass(row)]">
          {{ row.shortLabel }}
        </span>
        <div class="h-2 w-[72px] shrink-0 overflow-hidden rounded-full border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-800/70">
          <div
        ...
        <span :class="['w-9 shrink-0 text-right font-black', rowTextClass(row.usedPercent)]">
