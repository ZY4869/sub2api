...
describe('usageModelPresentation', () => {
  ...
  it('prefers million-context badge when requested and effective', () => {
    ...
    expect(presentation.badge?.label).toBe('1M')
    expect(presentation.badge?.muted).toBe(false)
  })

  it('renders million-context requested but not effective as muted 1M', () => {
    const presentation = buildUsageModelLinePresentation('deepseek-v4-pro', 'display_only', {
      million_context_requested: true,
    ...
    expect(presentation.badge?.label).toBe('1M')
    expect(presentation.badge?.muted).toBe(true)
  })

  it('falls back to registry context when no million-context flag is present', () => {
    ...
    expect(presentation.badge?.label).toBe('256K')
  })
  ...
  it('falls back to model id when display name lookup misses', () => {
    ...
    expect(presentation.badge).toBeNull()
  })
