...
export const useAppStore = defineStore("app", () => {
  ...
  function normalizeToastDetails(
    details?: ToastDetailInput[],
  ): ToastDetailItem[] | undefined {
    if (!details?.length) {
      return undefined;
    }
    return details
      .map((detail) => {
        ...
        return text ? { text, tone: detail.tone } : null;
      })
      .filter((detail): detail is ToastDetailItem => detail !== null);
  }
  ...
  function showToast(
    ...
    const duration = resolvedOptions.persistent
      ? undefined
    ...
    const toast: Toast = {
      ...
      details: normalizeToastDetails(resolvedOptions.details),
  ...
  function showWarning(
    message: string,
    options: number | ToastOptions = 4000,
  ): string {
