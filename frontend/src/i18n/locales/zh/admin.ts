export default {
  admin: {
    ...
    accounts: {
      ...
      batchCreateArchiveNotice: '归档模式会把账号直接创建为 inactive，并只绑定到归档组；普通分组与自动取模不会生效。',
      batchCreateArchiveGroupsDisabled: '当前处于归档模式，普通分组绑定已禁用，本批账号只会进入归档组。',
      batchCreateAutoImportDisabledHint: '归档账号会保持 inactive，因此不会在创建后自动取模。',
      ...
      bulkActions: {
        ...
        archive: '批量归档',
        archiveTitle: '批量归档账号',
        archiveDescription: '会将选中的 {count} 个账号直接设为 inactive，并只绑定到归档组，方便后续通过状态、分组和搜索管理。',
        archiveCurrentGroup: '归档当前分组',
        archiveCurrentGroupTitle: '归档当前分组',
        archiveCurrentGroupDescription: '会将当前分组 {group} 下的全部账号直接设为 inactive，并移动到归档组，不受分页和勾选影响。',
