export default {
    ...
    requestHeadersInvalidName: "请求头名称只能包含合法 HTTP token 字符。",
    requestHeadersForbiddenName: "该请求头不能被覆写。",
    ...
    poolMode: "池模式",
    poolModeHint: "上游为 API Key 账号池时启用，错误不会标记本地账号状态",
    poolModeInfo: "启用后，常见的 401/403/429 错误会在同一账号内自动重试，不会把账号直接标记为限流或错误。适用于 Anthropic、OpenAI 等 API Key 上游账号池。",
    ...
    poolModeRetryStatusCodesHint: "仅这些 HTTP 状态码会在同一账号内重试；留空或无效时使用默认 401、403、429。若上游池主要返回 5xx，可按实际情况手动补充。",
    ...
    autoRenewEnabled: "账号自动续费",
    autoRenewToggle: "到期自动顺延",
    autoRenewHint: "开启后，到期后台服务会从原到期时间顺延一个周期；不会自动恢复手动停用或错误状态。",
    autoRenewPeriod: "续费周期",
    autoRenewPeriodHint: "每轮最多续一次，按原到期日顺延。",
    autoRenewRequiresExpiration: "开启自动续费前需要设置过期时间。",
    autoRenewPeriodMonth: "每月",
    autoRenewPeriodQuarter: "每季度",
    autoRenewPeriodYear: "每年",
    expired: "已过期",
    proxy: "代理",
    noProxy: "无代理",
    accountTier: {
        label: "账号等级",
        hint: "等级会写入账号扩展信息，并用于新建或导入时快速套用默认容量。",
        applyCapacity: "套用默认容量",
        capacityHint: "该等级默认容量：{capacity}",
        options: {
            pro_20x: "Pro 20x（{capacity} 容量）",
            pro_5x: "Pro 5x（{capacity} 容量）",
            plus: "Plus（{capacity} 容量）",
            team: "Team（{capacity} 容量）",
            free: "Free（{capacity} 容量）",
            max_20x: "Max 20x（{capacity} 容量）",
            max_5x: "Max 5x（{capacity} 容量）",
            pro: "Pro（{capacity} 容量）",
        },
    },
    concurrency: "并发数",
    ...
    expirationQuickWeek: "7 天",
    expirationQuickMonth: "1 个月",
    expirationQuickYear: "1 年",
    expiresAtPreview: "将于 {value} 过期",
    expiryProbeExtensionDays: "到期自检顺延天数",
    expiryProbeExtensionDaysHint: "到期后若自检仍可用，将按这里的天数自动顺延到期时间。",
    expiryProbeExtensionQuick1d: "顺延 1 天",
    expiryProbeExtensionQuick7d: "顺延 7 天",
    ...
    creating: "创建中...",
    updating: "更新中...",
    accountCreated: "账号已保存，如需探测模型或验证连接，请手动执行 Probe/Test",
    autoImportModels: "创建后立即手动取模",
    probingModels: "探测模型中...",
    ...
    accountUpdated: "账号已保存，如需探测模型或验证连接，请手动执行 Probe/Test",
    failedToCreate: "创建账号失败",
    failedToUpdate: "更新账号失败",
    invalidBaseUrl: "保存失败：Base URL 不符合当前安全策略（ACCOUNT_INVALID_BASE_URL）",
    upstreamRedirectBlockedHint: "上游返回重定向会被拒绝，请改用最终 HTTPS 地址。",
    ...
    grokOauth: {
        title: "Grok 官方登录",
        description: "使用 xAI 授权链接或 Sub2api 设备码完成登录。系统会通过本地后端交换 token，并按本页设置创建 Grok OAuth 账号。",
        callbackMode: "浏览器回调",
        deviceMode: "设备码登录",
        generate: "生成授权链接",
        regenerate: "重新生成",
        ...
        callbackUrl: "回调地址或授权 Code",
        callbackPlaceholder: "粘贴 http://127.0.0.1:56121/callback?code=...&state=...，或只粘贴 code",
        callbackHint: "如果浏览器显示本地回调页无法访问，直接复制地址栏完整 URL 即可。",
        parsedCode: "已解析 Code",
        parsedState: "已解析 State",
        generateFailed: "生成 Grok 授权链接失败",
        exchangeFailed: "Grok 授权失败，请重新生成链接后再试",
        sessionMissing: "请先生成 Grok 授权链接",
        codeMissing: "请粘贴 Grok 授权回调地址或 code",
        accessTokenMissing: "Grok OAuth 未返回 access_token",
        externalDeviceCodeHint: "请先在 Sub2api 生成设备码；外部 Grok Build 短码不能由 Sub2api 兑换。",
        deviceStart: "生成设备码",
        deviceRegenerate: "重新生成设备码",
        openDeviceUrl: "打开验证页",
        copyDeviceCode: "复制短码",
        deviceUserCode: "设备短码",
        deviceUrl: "验证链接",
        deviceExpiresIn: "剩余时间",
        deviceStarted: "设备码已生成，请在 xAI 验证页完成登录。",
        deviceStartFailed: "生成 Grok 设备码失败",
        devicePollFailed: "查询 Grok 设备授权状态失败",
        devicePending: "Grok 设备授权仍在等待确认。",
        deviceSlowDown: "轮询过于频繁，已按 xAI 返回间隔放慢查询。",
        deviceDenied: "Grok 设备授权已被拒绝，请重新生成设备码。",
        deviceExpired: "Grok 设备码已过期，请重新生成设备码。",
        devicePollingHint: "自动按 {seconds} 秒间隔查询，也可以手动点击确认。",
    },
    ...
    openaiPlanTypeOverride: "OpenAI 方案类型",
    openaiPlanTypeOverridePlaceholder: "例如 plus、pro、team",
    openaiPlanTypeOverrideHint: "仅对 OpenAI OAuth 账号生效。留空会移除手动覆盖，让系统按账号返回信息判断能力。",
