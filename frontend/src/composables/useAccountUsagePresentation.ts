...
async function performUsageLoad(
  account: Account,
...
export function useAccountUsagePresentation(
  ...
  const geminiUserLevel = computed((): string | null => {
    ...
    if (
      geminiChannel.value === "ai_studio" ||
      geminiChannel.value === "ai_studio_client"
    ) {
  ...
  const geminiAuthTypeLabel = computed(() => {
    ...
    return levelLabel
      ? `${geminiChannelShort.value} ${levelLabel}`
      : geminiChannelShort.value;
  });
  ...
  const geminiQuotaPolicyLimits = computed(() => {
    ...
    if (geminiChannel.value === "google_one") {
      if (
        tierLower === "google_ai_ultra" ||
        geminiUserLevel.value === "ultra"
      ) {
        return t(
    ...
    if (geminiChannel.value === "gcp") {
      if (
        tierLower === "gcp_enterprise" ||
        geminiUserLevel.value === "enterprise"
      ) {
    ...
    if (geminiChannel.value === "ai_studio_client") {
      if (
        tierLower === "aistudio_tier_3" ||
        geminiUserLevel.value === "tier_3"
      ) {
        return t(
          "admin.accounts.gemini.quotaPolicy.rows.customOAuth.limitsTier3",
        );
      }
      if (
        tierLower === "aistudio_tier_2" ||
        geminiUserLevel.value === "tier_2"
      ) {
        return t(
          "admin.accounts.gemini.quotaPolicy.rows.customOAuth.limitsTier2",
        );
      }
      if (
        tierLower === "aistudio_tier_1" ||
        tierLower === "aistudio_paid" ||
        geminiUserLevel.value === "tier_1"
      ) {
        return t(
          "admin.accounts.gemini.quotaPolicy.rows.customOAuth.limitsPaid",
        );
      }
