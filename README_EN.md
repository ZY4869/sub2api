...
Override the release source repository if needed:
...
This repository already reuses `.github/workflows/release.yml` and `.goreleaser*.yaml` for release automation in the current GitHub repository context.
...
1. Create an annotated pre-release tag first:
   ...
   ```
2. Verify the `Release` workflow succeeds in GitHub Actions.
3. Verify GitHub Releases contains binaries and `checksums.txt`.
4. Verify GHCR contains `ghcr.io/zy4869/sub2api:v0.0.1-rc1`.
5. After validation, create the final annotated tag:
   ```bash
...
- `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` enable Telegram release notifications.
- If Docker Hub secrets are not configured, the workflow still publishes GitHub Releases and GHCR images.
